...
1. 参考 references/agent-config-guide.md 中的智能体配置文件说明，配置智能体的 `profile.md` 文件和 `memory.md` 文件

### 7. 使用方法

在飞书群聊中：
1. 上传打卡图片并发送消息：`打卡 @机器人`
2. 机器人会自动处理，包括下载图片、写入飞书多维表格等
3. 机器人在群聊中@打卡人回复：`@xxx，✅ 打卡完成！` 或 `@xxx，❌ 打卡失败！错误原因！`

### 8. 优化

1. 缺乏实际企业数据，无法测试打卡部门获取是否正常
2. 进/出场及打卡分类字段建议采用单选框，用户打卡填写表单获取，表单获取流程需要优化
3. 若打卡方式改为填写表单进行打卡，需重新编辑整体Skill逻辑，目前仅支持图片打卡。
